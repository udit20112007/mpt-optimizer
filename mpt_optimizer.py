...
GOLDSHARE.NS and SBISENSEX.NS removed: both legitimate NSE symbols, but
Yahoo Finance persistently failed to serve chart data for them across
multiple app runs, matching a known Yahoo backend issue (yfinance GitHub
#2052) unrelated to rate-limiting. Replaced with HDFCLOWVOL.NS and
MONIFTY500.NS to keep the count at 50.

Fund universe note: SPAXX and FZCXX removed (money market funds have zero
variance, breaking mean-variance optimization). Replaced with PRGFX/PRWCX.
...
INDIA_ETFS_50 = {
    ...
    "SETFNIF50.NS": "SBI ETF Nifty 50", "SETFNIFBK.NS": "SBI ETF Nifty Bank", "HDFCPVTBAN.NS": "HDFC Nifty Private Bank ETF", "AXISGOLD.NS": "Axis Gold ETF",
    "SBIETFQLTY.NS": "SBI Nifty Quality ETF", "ABSLNN50ET.NS": "Aditya Birla SL Nifty 50 ETF", "EQUAL50.NS": "Nippon India Nifty 50 Equal Weight ETF",
    ...
    "HDFCSENSEX.NS": "HDFC Sensex ETF", "TATAGOLD.NS": "Tata Gold ETF", "QGOLDHALF.NS": "Quantum Gold Fund",
    ...
    "HDFCLOWVOL.NS": "HDFC Nifty100 Low Volatility 30 ETF", "MONIFTY500.NS": "Motilal Oswal Nifty 500 ETF",
}
...
st.sidebar.caption("Start typing a company name or ticker \u2014 matching results filter live as you type.")
search_selected_labels = st.sidebar.multiselect("Search assets by name or ticker", options=list(SEARCH_DIRECTORY.keys()), default=[], placeholder="Type to search, e.g. 'reliance', 'AAPL', 'nifty bank'...")
searched_tickers = [SEARCH_DIRECTORY[lbl] for lbl in search_selected_labels]

st.sidebar.markdown("**\u2014 or \u2014**")
bucket_choices = st.sidebar.multiselect("Pull an entire universe (adds all its tickers)", list(ALL_UNIVERSES.keys()), default=[])
bucket_tickers = []
...
custom_tickers = []
if use_custom:
    custom_input = st.sidebar.text_input("Custom ticker (.NS suffix for NSE stocks, e.g. INFY.NS)", "")
...
if n_selected > MAX_ASSETS:
    st.sidebar.error(f"You selected {n_selected} assets, but only {MAX_ASSETS} can be optimized per run.")
...
st.sidebar.caption("The app always fetches each ticker's full available history and caches it for 24h.")

exclude_short_history = st.sidebar.checkbox("Auto-exclude tickers with unusually short history (avoid shrinking the whole window)", value=False)
if exclude_short_history:
    min_history_years = st.sidebar.number_input("Minimum years of history required to keep a ticker in this run", min_value=0.1, max_value=20.0, value=2.0, step=0.5)
else:
    min_history_years = 0.0

use_custom_asof = st.sidebar.checkbox("Use a custom 'as of' end date (point-in-time analysis)", value=False)
if use_custom_asof:
    asof_date = st.sidebar.date_input("As-of date (analysis uses only data up to and including this date)", value=pd.Timestamp.today().date(), max_value=pd.Timestamp.today().date())
else:
    asof_date = None

analysis_years = st.sidebar.number_input("Years of history to use, counting back from the as-of date (0 = use everything available)", min_value=0.0, max_value=50.0, value=0.0, step=0.5)
...
if run_backtest:
    test_fraction = st.sidebar.slider("Test period size (% of analysis window held out at the end)", min_value=10, max_value=50, value=25, step=5) / 100.0
else:
    test_fraction = 0.25
...
def _download_chunk_max(chunk):
    for attempt in range(MAX_RETRIES):
        try:
            data = yf.download(chunk, period="max", auto_adjust=True, progress=False, group_by="ticker", threads=True)
...
def fetch_full_history(tickers):
    ...
    for i in range(0, len(remaining), CHUNK_SIZE):
        ...
        if data is None:
            continue
        for tk in chunk:
            s = _extract_close(data, tk) if len(chunk) > 1 else (data["Close"] if "Close" in data.columns else None)
...
def optimize_max_sharpe(mean_rets, cov, rf, bounds):
    ...
    res = minimize(neg_sharpe, x0, args=(mean_rets, cov, rf), method="SLSQP", bounds=bounds, constraints=cons, options={"maxiter": 500})
    return res.x
...
def optimize_min_vol(mean_rets, cov, bounds, target_return=None):
    ...
    if target_return is not None:
        cons.append({"type": "eq", "fun": lambda w: portfolio_perf(w, mean_rets, cov)[0] - target_return})
    res = minimize(vol_objective, x0, args=(mean_rets, cov), method="SLSQP", bounds=bounds, constraints=cons, options={"maxiter": 500})
    return res
...
def explain_frontier_plain_language(mean_rets, cov, w_max_sharpe, ret_ms, vol_ms, sharpe_ms, w_min_vol, ret_mv, vol_mv, assets, name_lookup, n, rf):
    ...
    lines = []
    lines.append(f"**In plain terms:** out of the {n} investments you picked, spreading your money across all of them instead of holding just one cuts your risk by roughly **{risk_reduction:.0%}** on average \u2014 this is the core benefit of diversification: it smooths out the bumps without necessarily costing you return.")
    if top3_names:
        lines.append(f"The single **best risk-adjusted mix** (gold star) leans most heavily on **{', '.join(top3_names)}**. It's expected to grow about **{ret_ms:.1%} per year**, with typical up-and-down swings of about **\u00b1{vol_ms:.1%}** \u2014 think of that as the size of a normal good or bad year, not a hard ceiling or floor.")
    lines.append(f"The **safest mix** (blue diamond) trades some of that return away \u2014 about **{return_gain:.1%} per year less** \u2014 in exchange for noticeably calmer swings (**{abs(vol_gain):.1%}** less bounce). Neither mix is \"right\"; it depends on whether you'd rather chase higher growth or sleep easier during downturns.")
    lines.append("The red curve is the boundary of what's mathematically achievable with this exact set of assets \u2014 every dot below or to the right of it represents a worse deal. Nothing on this chart guarantees future performance; it's built entirely from how these assets moved in the past.")
    return "\n\n".join(lines)
...
def explain_backtest_plain_language(bt_results, best_strategy, beat_benchmark, test_days):
    ...
    lines = []
    if beat_benchmark:
        lines.append(f"**In plain terms:** when the \"optimal\" mix was actually put to the test on **{test_days} days of unseen data**, it earned about **{diff:+.1f} percentage points more** than simply splitting your money equally across every asset. That's a genuinely encouraging sign \u2014 it suggests the optimization found a real, lasting pattern in how these assets behave.")
    else:
        lines.append(f"**In plain terms:** when the \"optimal\" mix was actually put to the test on **{test_days} days of unseen data**, it earned about **{abs(diff):.1f} percentage points LESS** than simply splitting your money equally across every asset. This is a common and important finding: a strategy that looks best on historical data doesn't always keep winning on data it never saw.")
    lines.append(f"**{best_strategy}** ended up being the best performer in this specific test window. Running the backtest with a different test period size or as-of date can change which strategy comes out ahead.")
    return "\n\n".join(lines)
...
if run_btn:
    ...
    if dropped:
        st.warning(f"Could not fetch data for: {', '.join(sorted(dropped))} \u2014 excluded from optimization. Yahoo Finance sometimes rate-limits large batch requests (HTTP 429), or the ticker may have changed/delisted. Click **Run Optimization** again in 30\u201360 seconds, or reduce the selection.")

    if full_prices.empty or full_prices.shape[1] < 2:
        st.error("Could not fetch enough price data. Wait about a minute and click **Run Optimization** again, try a smaller ticker selection, or reduce Monte Carlo simulations to retry faster.")
        st.stop()
    ...
    if exclude_short_history and min_history_years > 0:
        ...
        if short_tickers:
            st.warning(f"Excluded {len(short_tickers)} ticker(s) with less than {min_history_years} years of history: {', '.join(f'{tk} ({NAME_LOOKUP.get(tk, tk)}, {per_ticker_years[tk]:.1f}y)' for tk in short_tickers)}")
    ...
    short_but_usable = history_table[(history_table["Years of History"] < requested_years_for_flag) & (history_table["Years of History"] >= min(1.0, requested_years_for_flag * 0.1))]

    min_days_needed = 60 if run_backtest else 30
    if prices.shape[0] < min_days_needed:
        st.error(f"Only {prices.shape[0]} trading days available in this window \u2014 too few for reliable statistics{' with backtesting enabled' if run_backtest else ''}. Pick a longer window, fewer years back, or remove short-history tickers from your selection.")
        st.stop()
...
- **Most diversifying pair:** {NAME_LOOKUP.get(min_pair[0], min_pair[0])} & {NAME_LOOKUP.get(min_pair[1], min_pair[1])} ({corr_no_diag.loc[min_pair]:.2f})
    ...
    if not short_but_usable.empty:
        ...
        st.dataframe(short_but_usable[["Ticker", "Name", "History Start", "Years of History"]].style.format({"Years of History": "{:.1f} yrs"}), hide_index=True)
        st.caption("These tickers have less history than your requested window, so the FINAL analysis window is capped by the shortest one among them.")

    with st.expander("\U0001F4CA Full history-length table for ALL selected tickers"):
        st.dataframe(history_table.style.format({"Years of History": "{:.1f} yrs"}), hide_index=True)
    ...
    stats_df = pd.DataFrame({"Name": [NAME_LOOKUP.get(a, a) for a in assets], "Annualized Return": mean_rets, "Annualized Volatility": np.sqrt(np.diag(cov))}, index=assets)
    st.dataframe(stats_df.style.format({"Annualized Return": "{:.2%}", "Annualized Volatility": "{:.2%}"}), height=min(35 * n + 40, 500))

    st.subheader("2. Correlation Matrix (Heatmap)")
    fig_corr = go.Figure(data=go.Heatmap(z=corr, x=assets, y=assets, colorscale="RdBu", zmid=0, colorbar=dict(title="Corr")))
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=vols_mc, y=rets_mc, mode="markers", marker=dict(size=3, color=sharpe_mc, colorscale="Viridis", colorbar=dict(title="Sharpe"), showscale=True), name="Monte Carlo Portfolios", opacity=0.5))
    fig.add_trace(go.Scatter(x=frontier_vols, y=target_returns, mode="lines", line=dict(color="red", width=3), name="Efficient Frontier (SLSQP)"))
    fig.add_trace(go.Scatter(x=[vol_ms], y=[ret_ms], mode="markers", marker=dict(color="gold", size=16, symbol="star", line=dict(color="black", width=1)), name="Max Sharpe Portfolio"))
    fig.add_trace(go.Scatter(x=[vol_mv], y=[ret_mv], mode="markers", marker=dict(color="blue", size=14, symbol="diamond", line=dict(color="black", width=1)), name="Min Volatility Portfolio"))
    fig.update_layout(xaxis_title="Annualized Volatility (Risk)", yaxis_title="Annualized Expected Return", template="plotly_white", height=600, legend=dict(orientation="h", yanchor="bottom", y=1.02))
    st.plotly_chart(fig, use_container_width=True)

    st.caption(f"Gold star = tangency portfolio (max Sharpe). Blue diamond = global minimum-variance portfolio. Red curve = efficient frontier via SLSQP. Universe: {n} assets, analysis window: {window_desc}. Mixed currencies (USD/INR) are not FX-adjusted.")

    st.markdown("#### \U0001F4A1 What This Chart Means")
    st.markdown(explain_frontier_plain_language(mean_rets, cov, w_max_sharpe, ret_ms, vol_ms, sharpe_ms, w_min_vol, ret_mv, vol_mv, assets, NAME_LOOKUP, n, risk_free_rate))
    ...
    if run_backtest:
        ...
        if len(train_prices) < 30 or len(test_prices) < 10:
            st.warning("Not enough data to split into a meaningful train/test period at this window size. Try a longer analysis window or a smaller test period percentage.")
        else:
            ...
            with st.spinner("Optimizing on training period and simulating out-of-sample performance..."):
                ...
                bt_results = pd.DataFrame({
                    ...
                    "Test-Period Return (%)": [(cum_ms.iloc[-1] - 1) * 100, (cum_mv.iloc[-1] - 1) * 100, (cum_eq.iloc[-1] - 1) * 100],
                    "Test-Period Ann. Vol (%)": [pd_ms.std() * np.sqrt(252) * 100, pd_mv.std() * np.sqrt(252) * 100, pd_eq.std() * np.sqrt(252) * 100],
                    "Max Drawdown (%)": [compute_drawdown(cum_ms) * 100, compute_drawdown(cum_mv) * 100, compute_drawdown(cum_eq) * 100],
                    "Sortino Ratio": [compute_sortino(pd_ms, rf_daily), compute_sortino(pd_mv, rf_daily), compute_sortino(pd_eq, rf_daily)],
                })

            st.markdown(f"**Training period:** {train_prices.index.min().date()} \u2192 {train_prices.index.max().date()} ({len(train_prices)} days) \u2014 weights optimized here, then FROZEN.\n\n**Test period (out-of-sample):** {test_prices.index.min().date()} \u2192 {test_prices.index.max().date()} ({len(test_prices)} days) \u2014 frozen weights applied here, unseen during optimization.")

            st.markdown("**\U0001F4B0 Return earned by each strategy during the test period:**")
            styled_bt = bt_results.style.format({"Test-Period Return (%)": "{:+.2f}%", "Test-Period Ann. Vol (%)": "{:.2f}%", "Max Drawdown (%)": "{:.2f}%", "Sortino Ratio": "{:.3f}"})
            try:
            ...
            fig_bt = go.Figure()
            fig_bt.add_trace(go.Scatter(x=cum_ms.index, y=(cum_ms - 1) * 100, name="Max Sharpe (trained)", line=dict(color="gold", width=2.5)))
            fig_bt.add_trace(go.Scatter(x=cum_mv.index, y=(cum_mv - 1) * 100, name="Min Volatility (trained)", line=dict(color="blue", width=2.5)))
            fig_bt.add_trace(go.Scatter(x=cum_eq.index, y=(cum_eq - 1) * 100, name="Equal-Weight Benchmark", line=dict(color="gray", width=2, dash="dash")))
            fig_bt.update_layout(title="Out-of-Sample Cumulative Return (%)", xaxis_title="Date", yaxis_title="Cumulative Return (%)", template="plotly_white", height=500, legend=dict(orientation="h", yanchor="bottom", y=1.02))
            ...
            st.caption(f"**{best_strategy}** had the highest out-of-sample return in this test period. The Max Sharpe portfolio {'beat' if beat_benchmark else 'did NOT beat'} the equal-weight benchmark here.")
    ...
    st.markdown("**\U0001F50D How search works:** type into the sidebar search box \u2014 it matches company names and ticker symbols across all 300 available assets.\n\n\U0001F4C5 **Full-history data policy:** every ticker's entire available price history is fetched and cached for 24 hours.\n\n\u26A0\uFE0F **Note:** your analysis window is capped by whichever selected ticker has the shortest available history. A full history-length table for every selected ticker is shown after you run.\n\n\U0001F553 **Custom analysis window:** run the optimization as if today were an earlier date.\n\n\U0001F9EA **Backtesting:** see how the optimized weights would have actually performed on unseen data, with the exact % return earned by each strategy shown in a table.\n\n\U0001F4A1 **Plain-language takeaways:** after each chart, a jargon-free summary of what it means.")
